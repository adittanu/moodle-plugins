#!/usr/bin/env python3
"""Convert Dali AI Plugins markdown guides to PDF.

Fixes over v1:
- Image placeholders rendered as labeled boxes
- Emoji replaced with text equivalents (no more black boxes)
- Table of contents has clickable internal links
- Bookmarks added for PDF navigation panel
"""
...
class ImagePlaceholder(Flowable):
    def __init__(self, alt, w=160*mm, h=35*mm):
        Flowable.__init__(self)
        self.alt, self.w, self.h = alt, w, h
    def wrap(self, aw, ah): return self.w, self.h + 2*mm
    def draw(self):
        c = self.canv
        c.setDash(3,3); c.setStrokeColor(BORDER); c.setLineWidth(1)
        c.rect(0, 0, self.w, self.h, fill=1, stroke=1)
        c.setDash()
        c.setFillColor(LIGHT_GREY); c.rect(0,0,self.w,self.h,fill=1,stroke=0)
        c.setFillColor(GREY); c.setFont(FONT_NAME, 16)
        c.drawCentredString(self.w/2, self.h/2+3*mm, '[ Image Placeholder ]')
        c.setFont(FONT_NAME, 8)
        c.drawCentredString(self.w/2, self.h/2-7*mm, self.alt[:80])
...
class BookmarkCanvas(Canvas):
    ...
    def bookmarkPage(self, key, *args, **kwargs):
        """Called by AnchorFlowable or bookmarkHorizontalAbsolute."""
        # Add to outline panel (once per key)
        if key not in self._outlineAdded:
            ...
            try:
                ...
                pass
        # Call the REAL Canvas.bookmarkPage (not self.bookmarkPage!)
...
def build_pdf(md_path, pdf_path, title, subtitle):
    ...
    story += [Spacer(1,30*mm), Paragraph(esc(title), st['DocTitle']),
              ...
              Paragraph(esc("Panduan lengkap untuk plugin Dali AI di Moodle. "
                  "Mencakup Dali AI Widget, AI Quiz Generator, AI Grading, dan AI Lesson Plan."), st['Body']),
              PageBreak()]
    ...
    def flush_tbl():
        ...
        t.setStyle(TableStyle([
            ...
            ('ROWBACKGROUNDS',(0,1),(-1,-1),[white,VL_GREY]),
    ...
    while i < len(lines):
        ...
        if m:
            ...
            story.append(Spacer(1,2*mm))  # image placeholder removed in v2
            i += 1; continue
...
def main():
    base = "D:/Project/dali/moodle-plugins"
    build_pdf(os.path.join(base,"GUIDE_ADMIN_DALI_PLUGINS.md"),
              os.path.join(base,"GUIDE_AI_PLUGINS_ADMIN.pdf"),
              "Dali AI Plugins \u2014 Panduan Admin",
              "Panduan konfigurasi dan administrasi untuk Admin Moodle")
    build_pdf(os.path.join(base,"GUIDE_TEACHER_DALI_PLUGINS.md"),
              os.path.join(base,"GUIDE_AI_PLUGINS_TEACHER.pdf"),
